...
nmf.initialization()
nmf.factorize()
word_topic_3  = nmf.W.T

#get topic top-words 
topic_word=get_top_word(top_k=50,word_topic=word_topic_3,num_bases=num_bases)
#save the result
handle_file().save_json("result/tree4/topic_word_weight_"+str(year_list[0])+".json" ,topic_word)
np.save("result/tree4/word_topic_"+str(year_list[0]),word_topic_3)
matrix_W=word_topic_3
...
nmf.initialization()
nmf.factorize()
word_topic_2  = nmf.W.dot(nmf.M).T
topic_word=get_top_word(top_k=50,word_topic=word_topic_2,num_bases=num_bases2)
handle_file().save_json("result/tree3/topic_word_weight_"+str(year_list[0])+".json" ,topic_word)
np.save("result/tree3/word_topic_"+str(year_list[0]),word_topic_2)
np.save("result/tree3/matrixM_"+str(year_list[0]),nmf.M)
matrix_W=word_topic_2
...
nmf.initialization()
nmf.factorize()
word_topic_1  = nmf.W.dot(nmf.M).T
topic_word=get_top_word(top_k=50,word_topic=word_topic_1,num_bases=num_bases3)
handle_file().save_json("result/tree2/topic_word_weight_"+str(year_list[0])+".json" ,topic_word)
np.save("result/tree2/word_topic_"+str(year_list[0]),word_topic_1)
np.save("result/tree2/matrixM_"+str(year_list[0]),nmf.M)
...
for date in year_list[1:]:
	...
	nmf =streamNMF(data=matrix.T, pre_word_topic=word_topic_3.T, num_bases=num_bases, niter=100)
	nmf.initialization()
	nmf.factorize()
	word_topic_3  = nmf.W.T
	topic_word=get_top_word(top_k=50,word_topic=word_topic_3,num_bases=num_bases)
	handle_file().save_json("result/tree4/topic_word_weight_"+str(date)+".json" ,topic_word)
	np.save("result/tree4/word_topic_"+str(date),word_topic_3)
	matrix_W=word_topic_3
	...
	word_topic_2  = nmf.W.dot(nmf.M).T
	topic_word=get_top_word(top_k=50,word_topic=word_topic_2,num_bases=num_bases2)
	handle_file().save_json("result/tree3/topic_word_weight_"+str(date)+".json" ,topic_word)
	np.save("result/tree3/word_topic_"+str(date),word_topic_2)
	np.save("result/tree3/matrixM_"+str(date),nmf.M)
	matrix_W=word_topic_2
	...
	word_topic_1  = nmf.W.dot(nmf.M).T
	topic_word=get_top_word(top_k=50,word_topic=word_topic_1,num_bases=num_bases3)
	handle_file().save_json("result/tree2/topic_word_weight_"+str(date)+".json" ,topic_word)
	np.save("result/tree2/word_topic_"+str(date),word_topic_1)
	np.save("result/tree2/matrixM_"+str(date),nmf.M)
